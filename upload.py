...
if args.dry_run:
    ...
    for file_path in args.files:
        url = f"{args.http_base}/tour/preview.html"
        print(f"===== Previewing {file_path} at {url}", flush=True)

        try:
            with open(file_path, 'rb') as f:
                t = json.load(f)
        except json.JSONDecodeError as e:
            print(f"Preview failed: {file_path} is not valid JSON: {e}", file=sys.stderr)
            failed = True
            continue
...
if args.password is not None:
    ...
    http_password = getpass.getpass(prompt=f'Password for {args.user}: ', stream=None)

for file_path in args.files:
    url = f"{args.http_base}/tour/data.json/{os.path.splitext(os.path.basename(file_path))[0]}"
    ...
    request.add_header("Authorization", f"Basic {base64.b64encode(f'{args.user}:{http_password}'.encode()).decode('utf8')}")
    request.add_header('Content-Type', 'application/json; charset=utf-8')
    ...
    request = urllib.request.Request(f"{args.http_base}/tour/data.html/{os.path.splitext(os.path.basename(file_path))[0]}", method='GET')
    try:
        with urllib.request.urlopen(request, bytes, context=ctx) as response:
            if response.status != 200:
                print("Tour rendered as HTML")
    except urllib.error.HTTPError as e:
        print(f"Tour cannot be rendered by data.html, look at OneZoom error logs: {e}", file=sys.stderr)
        sys.exit(1)
